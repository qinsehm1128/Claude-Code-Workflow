...
多维度代码审查，生成结构化报告。分析正确性、可读性、性能、安全性、测试覆盖和架构一致性。触发关键词："review code"、"code review"、"审查代码"、"代码审查"。
...
|------|-----|
| **名称** | `review-code` |
| **工具** | `Task`, `AskUserQuestion`, `Read`, `Write`, `Glob`, `Grep`, `Bash`, `mcp__ace-tool__search_context`, `mcp__ide__getDiagnostics` |

## 参考

多维度代码审查 Skill，从 6 个关键维度分析代码，生成包含可操作建议的结构化审查报告。
